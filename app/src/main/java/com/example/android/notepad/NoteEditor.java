...
public class NoteEditor extends Activity {
    ...
    private View.OnDragListener listener = new View.OnDragListener() {
        ...
        public boolean onDrag(View v, DragEvent event) {
            Toast.makeText(getApplicationContext(),"拖拽了",Toast.LENGTH_SHORT).show();
            //获取事件
            ...
            switch (action) {
                ...
                case DragEvent.ACTION_DRAG_LOCATION:
                    ...
                    Log.i(TAG, "拖拽的view在监听view中的位置:x =" + lastX + ",y=" + lastY);
                    break;
                case DragEvent.ACTION_DROP:
                    Log.i(TAG, "释放拖拽的view");
                    if("note".equals(v.getTag().toString())){
                        mText.setText(mText.getText()+dragStr);
                    }
                    break;
            }
            return false;
        ...
        mText.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                ...
                if (start < 0 || end < 0 || start == end) {
                    ...
                    dragStr = mText.getText().subSequence(start,end).toString();
                }
                //mText.setText(dragStr);
                Toast.makeText(getApplicationContext(),dragStr,Toast.LENGTH_SHORT).show();
                ClipData clipData = ClipData.newPlainText("drag", dragStr);
                View.DragShadowBuilder shadowBuilder = new View.DragShadowBuilder(mText);
                if (dragStr != null) {
                    mText.startDrag(clipData,shadowBuilder,null,0);
                }
                return false;
            }
        });
        mText.setOnDragListener(listener);
